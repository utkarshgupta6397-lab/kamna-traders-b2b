...
import { generateTransferReceiveSlip, TransferReceivePrintPayload } from '@/lib/print/slip-renderer';
import ThermalSlip from '@/components/thermal-preview/ThermalSlip';

export default function PrintTransferReceiveSlipClient({
  payload,
}: {
  payload: TransferReceivePrintPayload;
}) {
  const slipLines = useMemo(() => {
    return generateTransferReceiveSlip(payload);
  }, [payload]);
  ...
  return (
    <div className="space-y-6 max-w-lg mx-auto print:max-w-none print:mx-0 print:p-0">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4 flex items-center justify-between shadow-sm print:hidden">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-gray-900 text-sm">
              Print Receive Slip — {payload.transferNumber}
            </h2>
        ...
        <button
          onClick={handlePrint}
          className="px-4 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded-lg font-bold text-xs transition-all flex items-center gap-2 shadow-sm active:scale-95"
        >
          <Printer size={14} />
          Print Slip
        </button>
