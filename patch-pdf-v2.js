...
// 1. Update margins and layout constants
code = code.replace(/const margin = 18;/, 'const margin = 10;'); // 95% width on 210mm A4
code = code.replace(/const cNavy: \[number, number, number\]   = \[26,  39, 102\];/, 'const cNavy: [number, number, number]   = [30, 42, 120];'); // #1E2A78

// 2. Rewrite Header and KPIs for single statement
const headerStart = code.indexOf("  if (!isGroup) {");
const headerEnd = code.indexOf("    currentY = kpiY + 12 + 4; // Compact gap");
if (headerStart !== -1 && headerEnd !== -1) {
  const newHeader = `  if (!isGroup) {
    const logoH = 9; // Reduce height by 40%
  ...
  code = code.substring(0, headerStart) + newHeader + code.substring(headerEnd + "    currentY = kpiY + 12 + 4; // Compact gap".length);
}

// 3. Rewrite autoTable Configuration
...
// Replace existing didDrawPage in the main file loop? Wait, we can just hook into doc output or add the footer after autoTable.
// Currently the footer is handled by `didDrawPage` inside autoTable? No, earlier I saw an old chunk that used didDrawPage inside autoTable.
// Wait, the original code had:
//   if (finalTableY + spaceRequired > pageH && !options.isBatchRecovery) {
//     doc.addPage();
//   }
// Let's replace the final footer logic
