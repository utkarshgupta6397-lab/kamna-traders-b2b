...
For Vercel Postgres or Supabase poolers, `POSTGRES_PRISMA_URL` should be the pooled Prisma-compatible URL. The app and Prisma deploy config prefer `POSTGRES_PRISMA_URL` and fall back to `DATABASE_URL` for local setups. The Prisma schema intentionally does not use `directUrl` because Supabase's direct host is IPv6-only for this project and Vercel builds cannot reach it.
