...
export async function renderStatementToPdf(
  ...
  if (!isGroup) {
    ...
    const kpiHeight = 16; // KPIs and vertical padding
    const paymentBreakdownHeight = 6; // Approx 1-2 lines
    const bottomMargin = 38; // Includes Footer, QR, Amount Due, Generated By
    const gapBeforeTable = 4;
    
    const startY = 10 + headerHeight + kpiHeight + paymentBreakdownHeight + gapBeforeTable;
  ...
  if (!isGroup) {
    ...
    doc.text('Customer Statement', titleX, logoY + 7);
    
    doc.setFontSize(8);
    doc.setFont(pdfFont, 'normal');
    doc.setTextColor(...(theme === 'economy' ? [100, 116, 139] : [190, 205, 225] as [number, number, number]));
    doc.text('Kamna Traders · Receivables Ledger', titleX, logoY + 11.5);
    ...
    doc.text(cName, rightX, logoY + 5, { align: 'right' });

    doc.setFontSize(8);
    doc.setFont(pdfFont, 'normal');
    doc.setTextColor(...(theme === 'economy' ? [100, 116, 139] : [190, 205, 225] as [number, number, number]));
    if (s.customer.gstNo) doc.text(`GST: ${s.customer.gstNo}`, rightX, logoY + 9, { align: 'right' });
    if (s.customer.mobile) doc.text(`Phone: ${s.customer.mobile}`, rightX, logoY + 13, { align: 'right' });
    ...
    const kpiY = dividerY + 4;
    kpis.forEach((box, i) => {
      const bx = margin + i * (boxW + 8);
      const bh = 12; // Reduced height
      ...
      if (theme === 'economy') {
        ...
        const bg = box.color === cRed ? [254, 242, 242] : box.color === cGreen ? [236, 253, 245] : [248, 250, 252];
      ...
      doc.text(box.label.toUpperCase(), bx + 4, kpiY + 4.5);

      doc.setFontSize(9);
      doc.setTextColor(...box.color);
      doc.text(box.val, bx + 4, kpiY + 9.5);
    });

    currentY = kpiY + 12 + 4; // Compact gap
  ...
  autoTable(doc, {
    startY: currentY + 4, // Move table position slightly downward for breathing room
    head: tableHead,
    ...
    columnStyles: isGroup ? {
      ...
      4: { halign: 'right', cellWidth: 22, fontStyle: 'bold', fontSize: 6.5, overflow: 'visible' },
      5: { halign: 'right', cellWidth: 22, textColor: [5, 150, 105], fontStyle: 'bold', fontSize: 6.5, overflow: 'visible' },
      6: { halign: 'right', cellWidth: 24, fontStyle: 'bold', fontSize: 6.5, overflow: 'visible' },
    } : {
      0: { cellWidth: 22, overflow: 'visible' },
      1: { cellWidth: 16, overflow: 'visible' },
      2: { cellWidth: 58, overflow: 'linebreak' },
      3: { halign: 'right', cellWidth: 28, fontStyle: 'bold', fontSize: 6.5, overflow: 'visible' },
      4: { halign: 'right', cellWidth: 28, textColor: [5, 150, 105], fontStyle: 'bold', fontSize: 6.5, overflow: 'visible' },
    ...
    didParseCell: (data: any) => {
      ...
      if (data.section === 'body') {
        if (data.row.index === 0) {
          data.cell.styles.fillColor = theme === 'economy' ? [240, 249, 255] : [239, 246, 255];
