...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const url = `${API_BASE_URL}/books/v3/salesorders?organization_id=${orgId}&customer_id=${order.zohoBooksCustomerId}&sort_column=date&sort_order=D`;
    ...
    if (!res.ok) {
      ...
      console.error('[Zoho Fetch Sales Orders Error]', errText);
      try {
        const errJson = JSON.parse(errText);
        if (errJson.code === 57) {
          return NextResponse.json({ error: 'Missing OAuth scopes or Sales Orders module disabled' }, { status: 403 });
        }
        return NextResponse.json({ error: errJson.message || 'Failed to fetch sales orders' }, { status: res.status });
      } catch (e) {
        return NextResponse.json({ error: 'Failed to fetch sales orders from Zoho' }, { status: res.status });
      }
    }

    const data = await res.json();
    return NextResponse.json({ salesorders: data.salesorders || [] });

  } catch (error) {
    console.error('[Zoho Fetch Sales Orders Error]', error);
