...
import { CHART_COLORS, formatFYQuarter, formatMonth } from '@/lib/report-salesman';
...
function RevenueSectionComponent({ monthly, quarterly, systemType, callingExecRanking }: RevenueSectionProps) {
  ...
  const stackedBarChart = useMemo(() => {
    const topCallingExecs = callingExecRanking.slice(0, 6);
    const series = topCallingExecs.map((ce, i) => ({
      name: ce.name,
      type: 'bar',
      stack: 'total',
      barWidth: '40%',
      itemStyle: { color: CHART_COLORS[i % CHART_COLORS.length] },
      data: quarterly.map(q => q.byCallingExec[ce.id] || 0)
    }));
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 lg:col-span-2">
         <h2 className="text-sm font-semibold text-gray-700 mb-4">Sales by Calling Exec per Quarter</h2>
         <div className="h-[260px] w-full">
           {callingExecRanking.length > 0 && quarterly.length > 0 ? (
