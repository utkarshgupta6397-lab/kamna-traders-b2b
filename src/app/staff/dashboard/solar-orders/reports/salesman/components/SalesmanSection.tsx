...
import type { ReportData, SalesmanSummary } from '@/lib/report-salesman';
import { CHART_COLORS } from '@/lib/report-salesman';

interface SalesmanSectionProps {
  salesmanRanking: ReportData['salesmanRanking'];
  onSalesmanClick: (salesman: SalesmanSummary) => void;
}

function SalesmanSectionComponent({ salesmanRanking, onSalesmanClick }: SalesmanSectionProps) {
  const pieChart = useMemo(() => {
    let top8 = salesmanRanking.slice(0, 8);
    let others = salesmanRanking.slice(8);
    let data = top8.map((sm, i) => ({
      name: sm.name,
      value: sm.totalSales,
    ...
    if (others.length > 0) {
      const othersTotal = others.reduce((sum, sm) => sum + sm.totalSales, 0);
      data.push({
        name: 'Others',
        value: othersTotal,
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden lg:col-span-2 flex flex-col">
        <div className="px-5 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-gray-800">Salesman Performance</h2>
        </div>
            ...
            <tbody className="divide-y divide-gray-50 bg-white">
              {salesmanRanking.length === 0 ? (
                ...
                salesmanRanking.map((row, i) => {
                  ...
                  return (
                    <tr key={row.id} onClick={() => onSalesmanClick(row)} className="hover:bg-blue-50/40 cursor-pointer transition-colors">
...
export default memo(SalesmanSectionComponent);
