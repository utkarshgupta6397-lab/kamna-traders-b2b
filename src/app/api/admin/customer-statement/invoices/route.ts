...
export async function GET(request: Request) {
  ...
  if (!customerId || !/^\d+$/.test(customerId.trim()) || customerId.trim().length < 15) {
    return NextResponse.json({ error: 'Invalid or missing customerId. Must be a numeric ID (min 15 digits).' }, { status: 400 });
  }

  const result = await getCustomerInvoices(customerId.trim());
  if (!result.success) {
