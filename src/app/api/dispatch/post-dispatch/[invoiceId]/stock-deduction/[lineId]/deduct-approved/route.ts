  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (alloc.status === 'DEDUCTED') {
        return {
          allocation: { ...alloc, expectedQty: parseFloat(alloc.expectedQty.toString()) },
          historyIds: (alloc.inventoryHistoryIds as any) || [],
          isComplete: false,
          alreadyDeducted: true,
        };
      }
      if (alloc.status !== 'APPROVED') {
        throw new Error(`Cannot deduct: status is "${alloc.status}". Only APPROVED allocations can be deducted.`);
      }
    ...
    console.error('[DeductApproved]', err);
