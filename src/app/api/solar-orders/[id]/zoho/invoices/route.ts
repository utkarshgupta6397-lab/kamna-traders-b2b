...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const url = `${API_BASE_URL}/books/v3/invoices?organization_id=${orgId}&customer_id=${order.zohoBooksCustomerId}&sort_column=date&sort_order=D`;
    ...
    if (!res.ok) {
      ...
      console.error('[Zoho Fetch Invoices Error]', errText);
      try {
        const errJson = JSON.parse(errText);
        if (errJson.code === 57) {
          return NextResponse.json({ error: 'Missing OAuth scopes or Invoices module disabled' }, { status: 403 });
        }
        return NextResponse.json({ error: errJson.message || 'Failed to fetch invoices' }, { status: res.status });
      } catch (e) {
        return NextResponse.json({ error: 'Failed to fetch invoices from Zoho' }, { status: res.status });
      }
    }
