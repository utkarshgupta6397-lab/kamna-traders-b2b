...
const CallingAgentFilterPanel = dynamic(() => import('./components/CallingAgentFilterPanel'), { ssr: false });
...
const CallingAgentSection = dynamic(() => import('./components/CallingAgentSection'), { ssr: false });
...
const CallingAgentDrawer = dynamic(() => import('./components/CallingAgentDrawer'), { ssr: false });
...
// ------------------------------------
export default function CallingAgentReportClient() {
  ...
  // === Drill-down state ===
  const [selectedCallingExec, setSelectedCallingExec] = useState<CallingExecSummary | null>(null);
  ...
  useEffect(() => {
    ...
    fetch('/api/solar-orders/reports/salesman')
      ...
      .catch(err => {
        if (!cancelled) {
          console.error('[CallingAgentReport] Fetch error:', err);
  ...
  const handleCallingExecClick = useCallback((ce: CallingExecSummary) => {
    setSelectedCallingExec(ce);
  }, []);

  const handleDrawerClose = useCallback(() => {
    setSelectedCallingExec(null);
  }, []);

  const handleExportCSV = useCallback(() => {
    const timestamp = new Date().toISOString().split('T')[0];
    exportOrdersToCSV(filteredOrders, `calling_agent_report_${timestamp}.csv`);
  }, [filteredOrders]);
  ...
  return (
    <div className="min-h-full pb-16">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                Sales analytics across all calling executives
                {!loadingData && (
        ...
        <div className="px-6 space-y-6 pt-4">
          ...
          {(hasOrders || loadingData) && (
            <>
              ...
              <Section id="calling-agent">
                <CallingAgentSection
                  callingExecRanking={reportData.callingExecRanking}
                  onCallingExecClick={handleCallingExecClick}
                />
              </Section>
      ...
      {/* ─── Calling Agent Drill-Down Drawer ─── */}
      <CallingAgentDrawer
        callingExec={selectedCallingExec}
