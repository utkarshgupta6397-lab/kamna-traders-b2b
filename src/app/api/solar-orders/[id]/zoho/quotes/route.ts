...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const url = `${API_BASE_URL}/books/v3/estimates?organization_id=${orgId}&customer_id=${order.zohoBooksCustomerId}&sort_column=date&sort_order=D`;
    ...
    if (!res.ok) {
      ...
      console.error('[Zoho Fetch Quotes Error]', errText);
      try {
        const errJson = JSON.parse(errText);
        if (errJson.code === 57) {
          return NextResponse.json({ error: 'Missing OAuth scopes or Estimates module disabled' }, { status: 403 });
        }
        return NextResponse.json({ error: errJson.message || 'Failed to fetch quotes' }, { status: res.status });
      } catch (e) {
        return NextResponse.json({ error: 'Failed to fetch quotes from Zoho' }, { status: res.status });
      }
    }

    const data = await res.json();
    return NextResponse.json({ quotes: data.estimates || [] });

  } catch (error) {
    console.error('[Zoho Fetch Quotes Error]', error);
