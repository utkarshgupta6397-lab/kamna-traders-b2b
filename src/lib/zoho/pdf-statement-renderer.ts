...
export async function renderStatementToPdf(
  ...
  if (options.isBatchRecovery) {
    ...
    const headerHeight = 47; // From top of page to divider
    const kpiHeight = 24; // KPIs and vertical padding
    const paymentBreakdownHeight = 10; // Approx 1-2 lines
    const bottomMargin = 55; // Includes Footer, QR, Amount Due, Generated By
    const gapBeforeTable = 12;
    
    const startY = 14 + headerHeight + kpiHeight + paymentBreakdownHeight + gapBeforeTable;
    const availableHeight = pageHeight - startY - bottomMargin;
    
    // Measure one transaction row height
    const rowHeight = 7.5; // Compact row height with padding + font
  ...
  autoTable(doc, {
    ...
    margin: { top: 25, left: margin, right: margin, bottom: 55 },
