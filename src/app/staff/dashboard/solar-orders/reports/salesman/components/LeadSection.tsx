...
function LeadSection({ leadSource, callingExecRanking }: LeadSectionProps) {
  // Sort calling execs by totalSales desc
  const sortedExecs = useMemo(
    () => [...(callingExecRanking ?? [])].sort((a, b) => b.totalSales - a.totalSales),
    [callingExecRanking],
  );
  ...
  return (
    <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        ...
        {/* ── Right: Calling Executive Performance (1/3) ── */}
        <div className="flex flex-col">
          <p className="mb-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">
            Calling Executive Performance
          </p>
          ...
          {sortedExecs.length === 0 ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-gray-200 bg-gray-50 py-10">
              ...
              <p className="text-center text-xs text-gray-400">No calling activity data</p>
            </div>
          ) : (
