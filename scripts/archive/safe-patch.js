...
// 1. Setup margins & colors
code = code.replace(/const margin = 18;/, 'const margin = 10;'); // 95% width
code = code.replace(/const cNavy: \[number, number, number\]   = \[26,  39, 102\];/, 'const cNavy: [number, number, number]   = [30, 42, 120];');

// 2. Safely replace header block
const headerMarkerStart = "  if (!isGroup) {\\n    const logoH = 12;";
const headerMarkerEnd = "    currentY = kpiY + 12 + 4; // Compact gap\\n\\n  } else {";

const idx1 = code.indexOf("  if (!isGroup) {\\n    const logoH = 12;");
const idx2 = code.indexOf("    currentY = kpiY + 12 + 4; // Compact gap\\n\\n  } else {");
...
if (idx1 !== -1 && idx2 !== -1) {
  ...
  code = code.substring(0, idx1) + newHeader + code.substring(idx2 + "    currentY = kpiY + 12 + 4; // Compact gap\\n\\n  } else {".length - 9); 
} else {
  console.log("Could not find header boundaries.");
}

// 3. AutoTable replacement
const tIdx1 = code.indexOf("  const tableHead = [");
const tIdx2 = code.indexOf("  const closingBal = s.closingBalance;");
if (tIdx1 !== -1 && tIdx2 !== -1) {
...
\`;
  code = code.substring(0, tIdx1) + newTable + code.substring(tIdx2);
} else {
...
if (finalTableYStart !== -1 && oldFooterEnd !== -1) {
...
// Update row limit from 20 to 30 to allow 22-28 rows
