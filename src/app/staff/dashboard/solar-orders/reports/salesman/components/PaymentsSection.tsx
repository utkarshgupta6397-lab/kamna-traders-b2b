...
import { formatMonth } from '@/lib/report-salesman';
...
function PaymentsSectionComponent({ monthly, salesmanRanking, ageBuckets, paymentMode }: PaymentsSectionProps) {
  const stackedBarChart = useMemo(() => {
    const topSalesmen = salesmanRanking.slice(0, 10).reverse();
    return {
      tooltip: {
        trigger: 'axis',
      ...
      yAxis: { type: 'category', data: topSalesmen.map(sm => sm.name), axisLine: { lineStyle: { color: '#e5e7eb' } }, axisLabel: { color: '#6b7280', fontSize: 11, width: 90, overflow: 'truncate' }, axisTick: { show: false } },
      series: [
        { name: 'Paid', type: 'bar', stack: 'total', barWidth: '60%', itemStyle: { color: '#388E3C' }, data: topSalesmen.map(sm => sm.paidAmount) },
        { name: 'Pending', type: 'bar', stack: 'total', barWidth: '60%', itemStyle: { color: '#F57C00' }, data: topSalesmen.map(sm => sm.pendingAmount) }
      ]
    };
  }, [salesmanRanking]);
