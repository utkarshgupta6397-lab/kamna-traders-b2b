...
function CartRow({ item, index, updateQty, removeItem }: { 
  ...
  return (
    <div className="grid grid-cols-[14px_1fr_58px_30px_16px] gap-1.5 px-2 py-1 border-b border-[#F1F5F9] hover:bg-[#F1F5F9]/50 transition-colors group items-center">
      {/* 1. Sequence */}
      <div className="text-[9px] font-bold text-slate-300 tabular-nums">
        {index + 1}
      </div>

      {/* 2. Product Column — Dominant */}
      <div className="min-w-0 flex flex-col justify-center">
        <p className="text-[11px] font-[700] text-slate-900 leading-[1.1] break-words whitespace-normal" title={item.name}>
          {item.name}
        </p>
        <p className="text-[8px] text-slate-400 font-mono font-bold uppercase tracking-tight leading-none mt-0.5">
          {item.skuId}
        </p>
      </div>
      ...
      <div className="flex items-center gap-1">
        <button 
          ...
          className="w-[16px] h-[16px] flex items-center justify-center rounded bg-red-50 text-red-600 hover:bg-red-100 active:scale-90 transition-all"
        >
        ...
        <input
          ...
          className="w-[22px] h-[16px] text-center text-[10px] font-black text-slate-800 bg-slate-50 border border-slate-200 rounded outline-none tabular-nums focus:border-slate-400 focus:bg-white transition-colors"
          maxLength={3}
        />

        <button 
          onClick={() => updateQty(item.skuId, item.qty + (item.stepQty || 1))}
          className="w-[16px] h-[16px] flex items-center justify-center rounded bg-emerald-50 text-emerald-600 hover:bg-emerald-100 active:scale-90 transition-all"
        >
      ...
      <div className="text-[8px] font-bold text-slate-500 uppercase tracking-tighter opacity-70 truncate text-center">
...
export default function CartPanel() {
  ...
  return (
    <div className="flex flex-col h-full bg-white overflow-hidden select-none">
      ...
      <div className="bg-[#F8FAFC] border-b border-[#E2E8F0] px-2 py-1 grid grid-cols-[14px_1fr_58px_30px_16px] gap-1.5 items-center">
        ...
        <span className="text-[8px] font-black text-slate-400 uppercase tracking-tighter text-center">Qty</span>
        <span className="text-[8px] font-black text-slate-400 uppercase tracking-tighter text-center">UOM</span>
      ...
      {/* Bottom Summary */}
      <div className="p-2.5 border-t border-[#E2E8F0] bg-white flex-shrink-0">
        <div className="flex justify-between items-baseline mb-0.5">
          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Total Value</span>
          <span className="text-[16px] font-black text-slate-900 tabular-nums tracking-tighter">
